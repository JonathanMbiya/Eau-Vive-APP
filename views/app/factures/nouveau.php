...
<div class="flex flex-col items-center gap-10 text-gray-700 dark:text-gray-300 w-full">
	...
	renderHeader('Nouvelle Facture', 'Enregistrer une nouvelle facture',  [
		[
			'text' => 'Dashboard',
			'href' => '/app'
		],
	...
	<div class="mb-20 mt-4 flex flex-col w-full container mx-auto xl:max-w-7xl">
		...
		<div class="w-full p-6 md:p-10 rounded-lg bg-white dark:bg-gray-950 border border-gray-200 dark:border-gray-900">
			<form class="grid space-y-6" action="/app/factures/action" method="POST">
				<div class="grid gap-6 max-w-lg">
					<?php
					include("components/form.php");
					inputFormGroupLabel('Date facture', 'date', 'dateFacture', 'dateFacture', '', true);
					inputHidden('action_type', 'new-invoice');
					?>
				</div>
					...
					<div id="product-list" class="flex flex-col gap-y-4">
						<div data-row-invoice data-invoice-row-id="1" class="grid items-end gap-3 grid-cols-5 md:grid-cols-10 p-4 rounded-lg border border-gray-200 dark:border-gray-800/70">
							...
							<div class="col-span-2">
								...
								<input data-user-qte type="number" value="0" name="quantity[]" min="0" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-900/50 dark:border-gray-900 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" required>
							</div>
				...
				<div class="mt-6 pb-3 flex justify-end">
					<div class="space-y-2 p-5 rounded-lg border border-gray-200 dark:border-gray-900 w-full max-w-xs text-left">
						<h3 class="text-gray-600 dark:text-gray-400">Total</h3>
						<input data-total-invoice type="hidden" name="totalInvoice" value="0">
						<span class="text-3xl font-semibold text-gray-900 dark:text-white">
							<span data-span-total>0</span> Fc
						</span>
					</div>
				</div>
...
<?php
$qteEditable=true;
include("partials/invoiceScript.php");
?>
